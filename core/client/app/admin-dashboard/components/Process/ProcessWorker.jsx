...
class ProcessWorker extends Component {
  constructor(props) {
    super(props);
    this.state = {
      allProcessWorker: [],
  ...
  componentDidMount() {
    this.getListProcessWorker()
  }

  getListProcessWorker = () => {
    makeRequest("get", "/admin/process/getAllProcessWorker").then(result => {
      if (result.signal) {
        this.setState({
          allProcessWorker: result.data.allProcessWorker
        });
      }
    });
  }
  ...
  deleteProcessWorker(processWorker_id) {
    if (window.confirm("Bạn có chắc chắn muốn xóa!")) {
      makeRequest("post", "/admin/process/deleteProcessWorker", {
        id: processWorker_id
      })
        .then(result => {
          if (result.signal) {
        ...
        .catch(err => {
          console.log("Error in delete ProcessWorker", err);
        });
    }
  }

  renderProcessWorker = () => {
    let content = this.state.allProcessWorker.map((processWorker, idx) => {
      return (
        <tr key={"processWorker-" + idx}>
          <td>{processWorker.step_number}</td>
          <td>
            <img
              src={processWorker.image}
              className="img-circle img-xs ProcessWorker-left"
              alt=""
            />
          </td>
          <td>{processWorker.name}</td>
          <td>{processWorker.description.slice(0, 500)} ...</td>
          <td className="text-center">
            <div className="">
              <button
                type="button"
                className="label label-primary"
                onClick={this.clickEdit.bind(this, processWorker)}
              >
              ...
              <button
                ...
                onClick={this.deleteProcessWorker.bind(this, processWorker.id)}
              >
  ...
  clickEdit = processWorker => {
    this.setState({
      idEdit: processWorker.id,
      name: processWorker.name,
      description: processWorker.description,
      step_number: processWorker.step_number,
      image: processWorker.image,
  ...
  submitForm = e => {
    ...
    let url = "createProcessWorker";

    if (idEdit) {
      dataPost.id = idEdit;
      url = "updateProcessWorker";
    }
    ...
    makeRequest("post", `/admin/process/${url}`, dataPost).then(result => {
      if (result.signal) {
        ...
        this.cancelEdit();
        let { allProcessWorker } = this.state;
        if (idEdit) {
          this.setState(state => ({
            allProcessWorker: state.allProcessWorker.map(item => {
              if (item.id == idEdit) return result.data;
              return item;
            })
          }));
        } else {
          allProcessWorker.push(result.data);
          this.setState({
            allProcessWorker,
            idEdit:""
          });
  ...
  render() {
    return (
      <div>
        <PageHeader
          title="Quy trình đăng kí thợ"
          breadcrumb={[{ title: "ProcessWorker", link: "" }]}
        />
          ...
          <div className="row">
            <div className="col-xs-12 col-sm-12">
              ...
              <div className="col-md-8">
                <div className="panel panel-flat">
                  <div className="panel-body">{this.renderProcessWorker()}</div>
                </div>
              </div>
            </div>
          </div>
